...
_gc_init(E_Gadcon *gc, const char *name, const char *id, const char *style)
{
   ...
   tclock_config->instances = eina_list_append(tclock_config->instances, inst);
...
_gc_shutdown(E_Gadcon_Client *gcc)
{
   ...
   tclock_config->instances = eina_list_remove(tclock_config->instances, inst);
...
_tclock_cb_mouse_in(void *data, Evas *e, Evas_Object *obj EINA_UNUSED, void *event_info EINA_UNUSED)
{
   ...
   if (!(inst = data) || !inst->ci->show_tip || inst->tip)
     return;
   ...
   if (!id)
     {
        ...
        if (tclock_config->items)
          {
             const char *p;
             ci = eina_list_data_get(eina_list_last(tclock_config->items));
             p = strrchr(ci->id, '.');
             if (p) num = atoi(p + 1) + 1;
          }
     ...
     {
        EINA_LIST_FOREACH(tclock_config->items, l, ci)
          {
             if (!ci->id) continue;
...
e_modapi_shutdown(E_Module *m EINA_UNUSED)
{
   ...
   if (tclock_config->cfd)
     e_object_del(E_OBJECT(tclock_config->cfd));
   if (tclock_config->menu)
     {
   ...
   EINA_LIST_FREE(tclock_config->items, ci)
     {
        if (ci->id)
